...
#define kHorizontalPadding          2.0
#define kVerticalPadding            2.0
...
- (void)makeGooruToast:(NSString *)message duration:(CGFloat)interval position:(id)position image:(UIImageView *)image {
    UIView *toast = [self makeGooruViewForMessage:message title:nil image:image];
    [self showToast:toast duration:interval position:position];
}

- (void)makeToast:(NSString *)message duration:(CGFloat)interval pointPosition:(CGPoint)pointPosition {
    UIView *toast = [self makeGooruViewForMessage:message title:nil image:nil];
    [self showToast:toast duration:interval pointPosition:pointPosition];
}

- (void)showToast:(UIView *)toast duration:(CGFloat)interval pointPosition:(CGPoint)point {
    ...
    toast.frame = CGRectMake(toastPoint.x, toastPoint.y, toast.frame.size.width, toast.frame.size.height);
...
- (UIView *)makeGooruViewForMessage:(NSString *)message title:(NSString *)title image:(UIImageView *)image {
...
#endif
    [wrapperView setAutoresizingMask:(UIViewAutoresizingFlexibleLeftMargin | UIViewAutoresizingFlexibleRightMargin | UIViewAutoresizingFlexibleTopMargin | UIViewAutoresizingFlexibleBottomMargin)];
    [wrapperView.layer setCornerRadius:4.0];
    if (kDisplayShadow) {
        [wrapperView.layer setShadowColor:(__bridge CGColorRef)([UIColor colorWithRed:204.0/255.0 green:204.0/255.0 blue:204.0/255.0 alpha:1.0])];
        [wrapperView.layer setShadowOpacity:.25f];
        [wrapperView.layer setShadowRadius:1.0];
        [wrapperView.layer setShadowOffset:CGSizeMake(2.0, 2.0)];
    
    }

    [wrapperView setBackgroundColor:[[UIColor colorWithRed:51.0/255.0 green:51.0/255.0 blue:51.0/255.0 alpha:1.0] colorWithAlphaComponent:kOpacity]];
...
//        [imageView setFrame:CGRectMake(0, 0, kImageWidth, kImageHeight)];
    ...
    if(imageView != nil) {
        ...
        [wrapperView setFrame:CGRectMake(0.0, 0.0, image.frame.size.width, image.frame.size.height)];
        [wrapperView addSubview:image];
    }
