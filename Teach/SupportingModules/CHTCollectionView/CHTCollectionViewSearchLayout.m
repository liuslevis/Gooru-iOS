//
//  CHTCollectionViewSearchLayout.m
//  Gooru
//
...
@interface CHTCollectionViewSearchLayout()
...
@end
@implementation CHTCollectionViewSearchLayout

const int unionSize1 = 20;


#pragma mark - Accessors
- (void)setColumnCount:(NSUInteger)columnCount {
    
    NSLog(@"_columnCount=%lu",(unsigned long)_columnCount);
...
- (void)setItemWidth:(CGFloat)itemWidth {
    NSLog(@"_itemWidth=%lu",(unsigned long)_itemWidth);
...
- (void)prepareLayout {
    ...
    NSLog(@"itemCount=%lu",(unsigned long)_itemCount);
    _columnCount=4;
    _itemWidth = 160.0f;
    //_sectionInset = UIEdgeInsetsMake(2, 2, 2, 2);
    ...
    if ([self.delegate respondsToSelector:@selector(collectionView:heightForHeaderInLayout:)]) {
        CGFloat headerHeight = [self.delegate collectionViewSearch:self.collectionView
    ...
    for (idx = 0; idx < _itemCount; idx++) {
        ...
        CGFloat itemHeight = [self.delegate collectionViewSearch:self.collectionView
                                                    layout:self heightForItemAtIndexPath:indexPath];
    ...
    while (idx < _itemCount) {
        ...
        idx = MIN(idx + unionSize1, _itemCount) - 1;
    ...
    if ([self.delegate respondsToSelector:@selector(collectionView:heightForFooterInLayout:)]) {
        CGFloat footerHeight = [self.delegate collectionViewSearch:self.collectionView
...
- (NSArray *)layoutAttributesForElementsInRect:(CGRect)rect {
    ...
    for (i = self.unionRects.count - 1; i >= 0; i--) {
        if (CGRectIntersectsRect(rect, [self.unionRects[i] CGRectValue])) {
            end = MIN((i + 1) * unionSize1, self.itemAttributes.count);
            break;
        }
    }
    for (i = begin; i < end; i++) {
